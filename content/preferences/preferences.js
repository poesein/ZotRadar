...
window.ZotRadarPrefs = {
  ...
  applyStaticI18n(){
    ...
    for(const [id,key] of Object.entries({
      'zr-pref-save':'prefs.saveSettings','zr-pref-test-ollama':'prefs.testOllama','zr-pref-test-easyscholar':'prefs.testEasyScholar',
  ...
  bindActions(){if(this.actionsBound)return;this.actionsBound=true;for(const [id,method] of Object.entries({'zr-pref-save':'save','zr-pref-test-ollama':'testOllama','zr-pref-test-easyscholar':'testEasyScholar','zr-pref-open-dashboard':'openDashboard','zr-pref-system-refresh':'refreshSystemStatus','zr-pref-diagnostics-refresh':'refreshDiagnostics'})){document.getElementById(id)?.addEventListener('click',()=>{Promise.resolve().then(()=>this[method]()).catch(e=>{const status=document.getElementById('zr-pref-status');if(status)status.textContent=this.ZR?.I18n?.error(e)||String(e);try{Zotero.logError(e)}catch(_){}})})}},
  ...
  renderSettings(){
    ...
    for(const g of groups){
      ...
      for(const def of g.defs){
        const row=zrPrefsHTML('div'); row.className='zr-setting-row'; const label=zrPrefsHTML('label'); label.textContent=def.label; row.appendChild(label); let input;
        if(def.type==='boolean'){
  ...
  async testOllama(){ await this.save(); const s=document.getElementById('zr-pref-status'); s.textContent=this.t('prefs.testingOllama'); try{const r=await this.ZR.Ollama.health();s.textContent=r.ok?this.t('prefs.ollamaConnected',{models:(r.models||[]).join(', ')}):this.t('prefs.ollamaFailed',{error:r.error});}catch(e){s.textContent=this.t('prefs.ollamaFailed',{error:String(e&&e.message||e)});} },
