(function (ZR) {
  ...
  async function init(){try{Zotero.ftl.addResourceIds(['zotradar.ftl']);}catch(_){}await registerColumns();registerPane();paneUnsubs.push(ZR.Events.on('feedback:changed',e=>{if(!e?.snapshotOnly)refreshPanes(e)}),ZR.Events.on('papers:changed',e=>refreshPanes(e)));}
