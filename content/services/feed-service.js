(function(ZR){'use strict';
 ...
 function validate(x,existingID=null){
   ...
   const name=String(x.name||'').trim(),type=String(x.type||'').trim(),url=String(x.url||'').trim();
   ...
   return {...x,id,name,type,url,enabled:x.enabled!==false,max_items:max,query:String(x.query||''),query_note:String(x.query_note||'')};
 }
 ...
 async function test(id){const f=await get(id);const r=await ZR.Feeds.fetchAll([f.id]);return {ok:true,fetched:r.fetched,errors:r.errors,papers:(r.papers||[]).slice(0,5).map(p=>({title:p.title,url:p.url}))};}
