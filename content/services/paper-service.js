(function(ZR){'use strict';
 ...
 async function processTitleTranslations(){
   ...
   try{while(translationQueue.length){
     ...
     try{
       ...
       const model=ZR.Settings.get('screeningModel');
       const resp=await ZR.Ollama.request('/api/chat',{model,messages:[{role:'system',content:'Translate this scientific paper title into concise Simplified Chinese. Preserve gene names, proteins, mutations, numbers, Greek letters and method names exactly. Do not add facts. Return JSON only.'},{role:'user',content:p.title}],stream:false,format:{type:'object',required:['title_zh'],properties:{title_zh:{type:'string'}}},think:false,options:{temperature:0}});
       const raw=String(resp?.message?.content||'').trim(),parsed=ZR.Utils.safeJSON(raw,null);
 ...
 async function list(opts={}){
   ...
   let rows=(await ZR.DB.latestSubscriptionScreenings(sid,all?2147483647:Number(opts.limit||10000))).filter(r=>r.scorecard_id===sub.scorecard).map(r=>{const p=shape(r);p.preprint_source=ZR.Journal.preprintSource?.({...p,source_records:ZR.Utils.safeJSON(r.source_records_json,[])})||null;p.is_preprint=!!p.preprint_source;return p});
 ...
 async function forZoteroItem(item){const paper=await ZR.DB.paperByZotero(item.libraryID,item.key);if(!paper)return{paper:null,screening:null};const subID=String(ZR.Utils.getPref('defaultSubscription','protein_design'));const sub=(ZR.Config.subscriptions.subscriptions||[]).find(x=>x.id===subID&&x.enabled!==false)||(ZR.Config.subscriptions.subscriptions||[]).find(x=>x.enabled!==false);if(!sub)return{paper,screening:null};const rows=await ZR.DB.latestSubscriptionScreenings(sub.id,100000);const r=rows.find(x=>Number(x.paper_id)===Number(paper.id)&&x.scorecard_id===sub.scorecard);return{paper,screening:r?shape(r):null,subscription:sub};}
 ...
 async function requireScreeningModel(){if(!ZR.Ollama?.health)return;const state=await ZR.Ollama.health();if(!state.ok)throw new Error('Ollama unavailable: '+String(state.error||'connection failed'));const model=String(ZR.Utils.getPref('screeningModel','qwen3:8b'));if(Array.isArray(state.models)&&state.models.length&&!state.models.includes(model))throw new Error('Screening model not installed: '+model);}
