...
assert.equal(manifest.version, '5.4.1');
...
assert.match(read('prefs.js'), /modelProvider", "ollama"/);
assert.match(read('prefs.js'), /apiKey", ""/);
...
const allowedTop = new Set([
  ...
  'README.md', 'README_EN.md', 'RELEASE_AUDIT.md', 'updates.json', 'tests', 'releases', 'LICENSE'
]);
for (const name of fs.readdirSync(root)) assert.ok(name === '.git' || allowedTop.has(name), `Unexpected public root entry: ${name}`);
assert.deepEqual(fs.readdirSync(path.join(root, 'tests')).sort(), ['feed-sources.test.mjs', 'model-providers.test.mjs', 'pipeline-regressions.test.mjs', 'public-release.test.mjs', 'reasoning-effort.test.mjs', 'scoring.test.mjs', 'title-hygiene.test.mjs']);
...
walk(root);
console.log('Local 5.4.1 config, file-scope, and privacy smoke tests: PASS');
